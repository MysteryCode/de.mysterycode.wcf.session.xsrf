...
final class SessionHandler extends SingletonFactory
{
    ...
    {
        $xsrfToken = '';
        if (!empty($_COOKIE[COOKIE_PREFIX . 'XSRF-TOKEN'])) {
            ...
            if (!PACKAGE_ID || CryptoUtil::validateSignedString($_COOKIE[COOKIE_PREFIX . 'XSRF-TOKEN'])) {
                $xsrfToken = $_COOKIE[COOKIE_PREFIX . 'XSRF-TOKEN'];
            }
        }
        ...
        if (!$xsrfToken) {
            ...
            \header(
                'set-cookie: ' . COOKIE_PREFIX . 'XSRF-TOKEN=' . \rawurlencode($xsrfToken) . '; path=/' . $cookieDomain . (RouteHandler::secureConnection() ? '; secure' : '') . $sameSite,
                false
            );
        }
